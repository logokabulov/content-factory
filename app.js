...
$("authBtn").addEventListener("click", async () => {
  ...
  if (isLogin) {
    ...
    if (error) { showErr("authError", error.message); $("authBtn").disabled=false; $("authBtn").textContent="Kirish"; }
  } else {
    const confirm = $("authConfirm").value;
    if (password !== confirm) { showErr("authError","Parollar mos emas"); $("authBtn").disabled=false; $("authBtn").textContent="Ro'yxatdan o'tish"; return; }
    const { error } = await sb.auth.signUp({ email, password });
    if (error) { showErr("authError", error.message); }
    else { $("authMsg").textContent="✓ Emailingizni tasdiqlang"; $("authMsg").style.display="block"; }
    $("authBtn").disabled=false; $("authBtn").textContent="Ro'yxatdan o'tish";
  }
});
...
function renderProjects() {
  ...
  list.innerHTML = "";
  if (!projects.length) { list.innerHTML='<div class="text2 text-sm" style="padding:4px 10px">Hali proyekt yo\'q</div>'; return; }
  projects.forEach(p => {
    const div = document.createElement("div");
    div.className = "project-item" + (currentProject?.id===p.id?" active":"");
    div.innerHTML = `<span class="project-item-name">📁 ${p.name}</span><button class="project-delete" data-id="${p.id}">✕</button>`;
    div.querySelector(".project-item-name").addEventListener("click", () => selectProject(p));
    div.querySelector(".project-delete").addEventListener("click", e => { e.stopPropagation(); deleteTargetId=p.id; show("deleteModal"); });
...
$("addProjectBtn").addEventListener("click", () => {
  $("projectName").value=""; $("projectNiche").value=""; $("projectAudience").value="";
  show("newProjectModal"); setTimeout(()=>$("projectName").focus(),100);
});
...
$("saveProjectBtn").addEventListener("click", async () => {
  ...
  if (!error && data) { projects.unshift(data); renderProjects(); selectProject(data); hide("newProjectModal"); }
});
...
$("confirmDeleteBtn").addEventListener("click", async () => {
  ...
  if (currentProject?.id===deleteTargetId) { currentProject=null; $("mainTitle").textContent="Proyekt tanlang"; show("emptyState"); hide("contentForm"); }
  projects = projects.filter(p=>p.id!==deleteTargetId);
  deleteTargetId=null; hide("deleteModal"); renderProjects();
});
...
$("navSettings").addEventListener("click", ()=>{
  ...
  ["tabCreate","tabHistory","tabResearch"].forEach(id=>{ const el=$(id); if(el) el.classList.add("hidden"); });
...
//  GENERATE
// ══════════════════════════════════════════════════════════
$("topic").addEventListener("input", function(){ $("charCount").textContent=this.value.length+" belgi"; });
$("genBtn").addEventListener("click", async () => {
  ...
  hideErr("errorBox");
  const btn = $("genBtn");
  btn.disabled=true; btn.innerHTML='<span class="spinner"></span> Yaratilmoqda...';
  $("results").innerHTML="";
  try {
    const res = await fetch("/api/generate",{
      method:"POST", headers:{"Content-Type":"application/json"},
      body: JSON.stringify({
        name: currentProject.name, niche: currentProject.niche||"",
        audience: currentProject.audience||"", tone: $("tone").value,
        language: $("language").value, topic, extra: $("extra").value.trim(), platforms: selected,
      }),
    });
    ...
    await sb.from("contents").insert({
      user_id: currentUser.id, project_id: currentProject.id,
      topic, platforms: selected, result: parsed,
    });
  } catch(err) { showErr("errorBox","Xato: "+err.message); }
  finally { btn.disabled=false; btn.innerHTML="✦ Kontent yaratish"; }
});

function parseContent(text, platforms) {
  const keys = { threads:"THREADS POST", instagram:"INSTAGRAM KARUSEL", telegram:"TELEGRAM POST", reels:"REELS" };
  const result = {};
  platforms.forEach((pid,i) => {
    const key   = keys[pid]||pid.toUpperCase();
    const regex = new RegExp(`##\\s*${key}[^\\n]*\\n([\\s\\S]*?)(?=##\\s*[A-Z]|$)`,"i");
    const m     = text.match(regex);
    result[pid] = m ? m[1].trim() : (text.split(/##\s+/)[i+1]||text).replace(/^[A-Z\s\/()0-9]+\n/,"").trim();
  });
  return result;
}
...
async function loadHistory() {
  ...
  const { data } = await sb.from("contents").select("*").eq("project_id",currentProject.id).order("created_at",{ascending:false});
  if (!data||!data.length) { $("historyList").innerHTML=""; show("historyEmpty"); return; }
  hide("historyEmpty");
  const list = $("historyList");
  list.innerHTML="";
  data.forEach(item => {
    const platforms=(item.platforms||[]).map(pid=>{ const p=CONFIG.platforms.find(x=>x.id===pid); return p?p.emoji:pid; }).join(" ");
    const div=document.createElement("div");
    ...
    div.addEventListener("click",()=>{
      ...
      if (div.classList.contains("open")&&!detail.innerHTML) {
        ...
        (item.platforms||[]).forEach(pid=>{
          ...
          const info=CONFIG.platforms.find(p=>p.id===pid)||{emoji:"",name:pid};
          const card=document.createElement("div");
          card.className="result-card"; card.style.marginTop="10px";
          card.innerHTML=`
            <div class="result-header">
              <div class="result-platform">${info.emoji} ${info.name}</div>
...
function updateContentTypes() {
  ...
  select.innerHTML="";
  const types={
    instagram:[{value:"reels",label:"Reels"},{value:"carousel",label:"Karusel"},{value:"post",label:"Oddiy post"}],
    youtube:  [{value:"shorts",label:"Shorts"},{value:"video",label:"Oddiy video"}],
    threads:  [{value:"post",label:"Post"},{value:"reply",label:"Javoblar"}],
  };
  (types[platform]||[]).forEach(t=>{ const o=document.createElement("option"); o.value=t.value; o.textContent=t.label; select.appendChild(o); });
}
$("researchPlatform").addEventListener("change", updateContentTypes);

async function loadSavedCount() {
  if (!currentProject) { $("savedCount").textContent="0"; return; }
  const { count } = await sb.from("research_results").select("*",{count:"exact",head:true}).eq("project_id",currentProject.id);
...
$("researchBtn").addEventListener("click", async () => {
  ...
  btn.disabled=true; btn.innerHTML='<span class="spinner"></span> Qidirilmoqda... (30-45 sek)';
  $("researchResults").innerHTML="";
  ...
  try {
    ...
    const data = await res.json();
    if (!res.ok) throw new Error(data.error||"Xato");
    renderResearch(data.results, platform, data.warning||"", data.total||0);
  } catch(err) { showErr("researchError","Xato: "+err.message); }
  finally { btn.disabled=false; btn.innerHTML="🔍 Trendlarni topish"; }
});

function renderResearch(results, platform, warning, total) {
  const wrap=$("researchResults");
  if (!results||!results.length) {
    wrap.innerHTML='<div class="empty-state"><div class="icon">🔍</div><h3>Natija topilmadi</h3><p>Boshqa hashtag yoki platformani sinab ko\'ring</p></div>';
    return;
  }
  const platformEmoji={instagram:"📱",youtube:"🎬",threads:"💬"};
  wrap.innerHTML=`<div class="text-sm text2" style="margin-bottom:8px">${results.length} ta trend topildi ${total>results.length?"("+total+" dan)":""}</div>`;
  if (warning) {
    wrap.innerHTML+=`<div class="error-box" style="display:block;margin-bottom:12px">⚠️ ${warning}</div>`;
  }

  results.forEach((item,i)=>{
    const card=document.createElement("div");
    card.className="history-item"; card.style.cursor="default";
    card.innerHTML=`
    ...
    card.querySelector(".save-research-btn").addEventListener("click", async e=>{
      ...
      const saveBtn=e.target; saveBtn.disabled=true; saveBtn.textContent="...";
      const { error } = await sb.from("research_results").insert({
        user_id: currentUser.id, project_id: currentProject.id,
        platform: item.platform, content_type: item.contentType,
        title: item.title, views: item.views, likes: item.likes, url: item.url,
      });
      if (error) { saveBtn.disabled=false; saveBtn.textContent="⭐ Saqlash"; }
      else { saveBtn.textContent="✓ Saqlandi"; saveBtn.style.color="var(--success)"; loadSavedCount(); }
    });
    card.querySelector(".use-idea-btn").addEventListener("click",()=>{
      $("topic").value=item.title; $("charCount").textContent=item.title.length+" belgi";
      switchTab("create");
...
$("savedResearchBtn").addEventListener("click", async ()=>{
  hide("researchFormCard"); $("researchResults").innerHTML="";
  show("savedResearchPanel"); await loadSavedResearch();
});
$("closeSavedBtn").addEventListener("click",()=>{ hide("savedResearchPanel"); show("researchFormCard"); });

async function loadSavedResearch() {
  if (!currentProject) return;
  const list=$("savedResearchList");
  list.innerHTML='<div class="text2 text-sm">Yuklanmoqda...</div>';
  const { data } = await sb.from("research_results").select("*").eq("project_id",currentProject.id).order("created_at",{ascending:false});
  if (!data||!data.length) {
  ...
  data.forEach(item=>{
    ...
    card.querySelector(".del-saved-btn").addEventListener("click", async e=>{
      await sb.from("research_results").delete().eq("id",e.target.dataset.id);
      card.remove(); loadSavedCount();
    });
...
$("changePasswordBtn").addEventListener("click", async ()=>{
  const pw=$("newPassword").value, pw2=$("newPasswordConfirm").value;
...
async function logout() { await sb.auth.signOut(); currentUser=null; currentProject=null; projects=[]; }
...
// ══════════════════════════════════════════════════════════
function bindEvents() {
  $("newProjectModal").addEventListener("click", e=>{ if(e.target===$("newProjectModal")) hide("newProjectModal"); });
  $("deleteModal").addEventListener("click", e=>{ if(e.target===$("deleteModal")) { hide("deleteModal"); deleteTargetId=null; } });
  $("projectName").addEventListener("keydown", e=>{ if(e.key==="Enter") $("saveProjectBtn").click(); });
}
